...
def edgeListParser():
    ...
    edge_list_parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Input/Output argument
    edge_list_parser.add_argument(
        "--input",
        help="Defines the input filename",
        type=str,
    ...
    edge_list_parser.add_argument(
        ...
        type=str,
        default="edge_list.out",
    )
    type_list = ("tsv", "csv", "ssv")
    edge_list_parser.add_argument(
        "--output-type",
        metavar=metavarList(type_list),
    ...
    edge_list_parser.add_argument(
        ...
        help="Defines the euclidean distance cutoff",
        type=int,
        default=300,
    )
    edge_list_parser.add_argument(
        "--sample-size-cutoff",
        help="Defines the sample size cutoff",
        type=int,
        default=250,
    )
...
def tagEuclideanDistance(row_list):
    ...
    unique_tags = set()
    unique_row_list = [
        _r for _r in row_list if _r[0] not in unique_tags and not unique_tags.add(_r[0])
    ]
    ...
    for tag1, tag2 in itertools.combinations(unique_row_list, 2):
        yield (tag1[0], tag2[0]), np.linalg.norm(
            np.array(tag1[1:]) - np.array(tag2[1:])
        )
...
def inputToEuclideanDistance(input_file, distance_cutoff, sample_size_cutoff):
    ...
    with open(input_file) as csv_file:
        ...
        if not csv_file.readline().split(",")[0]:
            col_offset = 1
        else:
            col_offset = 0
        ...
        for csv_row in csv_reader:
            ...
            # Assign the row
            input_frame_pos, input_tag, input_x, input_y = (
                int(csv_row[0 + col_offset]),
                csv_row[1 + col_offset],
                float(csv_row[2 + col_offset]),
                float(csv_row[3 + col_offset]),
            )
            ...
            if not current_frame_pos or current_frame_pos != input_frame_pos:
                ...
                if current_frame_data and len(current_frame_data) > 1:
                    for tag_pair, tag_ed in tagEuclideanDistance(current_frame_data):
                        ...
                        if tag_ed < distance_cutoff:
                            tagEDSigCount[tag_pair] += 1
        ...
        for tag_pair, all_count in tagEDAllCount.items():
            if tag_pair not in tagEDSigCount or all_count < sample_size_cutoff:
                continue
...
def main():
    ...
    for tag1, tag2, euclidean_distance in inputToEuclideanDistance(
        edge_list_args.input,
        edge_list_args.euclidean_distance_cutoff,
        edge_list_args.sample_size_cutoff,
    ):

        # Write the returned (and filtered) euclidean distance to the edge list
        if edge_list_args.output_type == "tsv":
            edge_list_output.write(f"{tag1}\t{tag2}\t{euclidean_distance}\n")
        elif edge_list_args.output_type == "csv":
            edge_list_output.write(f"{tag1},{tag2},{euclidean_distance}\n")
        elif edge_list_args.output_type == "ssv":
            edge_list_output.write(f"{tag1} {tag2} {euclidean_distance}\n")
        else:
            raise Exception(
                f"Unable to assign output file type: {edge_list_args.output_type}"
            )
