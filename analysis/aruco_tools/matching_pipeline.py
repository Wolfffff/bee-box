...
def rolling_hungarian_matching(cleaned_aruco_csv_path: str, slp_predictions_path: str, rolling_window_size: int, pairing_threshold: float, sigmoid_inflection: float, sigmoid_slope: float, min_track_detection_freq: float, linear_dist_weight: float, enhanced_ouput: bool = False) -> list:
	...
	while current_frame < last_frame:
			...
			for instance in iter_frame_instances:
				...
				if (not np.isnan(pX)) and (not np.isnan(pY)):
					...
					if instance_number != -1:
						temp_cost_matrix[tracks_indices[instance_number], tags_indices[int(row.Tag)]] += sigmoid(distance, sigmoid_inflection, sigmoid_slope) + (linear_dist_weight * distance)
		...
		while i < len(for_hungarian):
			if np.sum(detection_count_matrix[i]) < (float(rolling_window_size * len(tags)) * min_track_detection_freq):
				print(f'Trimmed track {trimmed_tracks[i]} due to low detection count... {np.sum(detection_count_matrix[i])} detections.')
				for_hungarian = np.delete(for_hungarian, i, 0)
				detection_count_matrix = np.delete(detection_count_matrix, i, 0)
				trimmed_tracks = np.delete(trimmed_tracks, i)
			else:
		...
		# Eliminate zero-detection columns and columns not fulfilling the minimum track detection frequency
		while i < len(for_hungarian):
			if np.sum(for_hungarian[i]) == 0:
				for_hungarian = np.delete(for_hungarian, i, 0)
				detection_count_matrix = np.delete(detection_count_matrix, i, 0)
...
if __name__ == '__main__':
	...
	ArUco_csv_path    = str(sys.argv[3])    # The filepath for the ArUco tag data
	files_folder_path = str(sys.argv[4])	# The folder which is to contain the plethora of files that this script will spit out.  Please make this ahead of time!
	name_stem         = str(sys.argv[5])	# A basic name stem that will be conjugated to generate the various filenames.
	annotate_video    = bool(sys.argv[6])	# Should we annotate the video with the final pairings
	end_here_frame    = int(sys.argv[7])	# Last frame that should be processed before moving to next step
	...
	ArUco_tag_ignore_threshold = 50.        # An ArUco tag must be detected on at least (ArUco_tag_ignore_threshold)% of the video frames to be considered
	...
	rolling_Hungarian_window_length = 50    # Length of the rolling window over which tag-track matching costs will be summed for Hungarian matching.
	...
	maximum_assignment_cost_threshold = 30. # Threshold for the maximum cost of a tag-track combination to be actually outputted as a pairing
	...
	cost_linear_dist_weight = 0.5 / 3000.   # TODO: write more on this
											# The linear component of the weighting function

	min_track_detection_freq = 0.5          # TODO: write more on this
											# Minimum frequency of track detection within window

	tracklet_merging_change_penalty = 70.	# In the tracklet merging step, the penalty for changing tracks.
	...
	cost_sigmoid_inflection_dist_mt = 50.   # Same concept as cost_sigmoid_inflection_distance, just for tracklet end/start points
	...
	with open(files_folder_path + '/' + name_stem + '_console_output.txt', 'w') as f:
		with redirect_stdout(f):
			...
			print(f'Parameters:\n{ArUco_tag_ignore_threshold}, {rolling_Hungarian_window_length}, {cost_sigmoid_inflection_distance}, {cost_sigmoid_inflection_slope}, {maximum_assignment_cost_threshold}, {cost_linear_dist_weight}, {min_track_detection_freq}, {tracklet_merging_change_penalty}, {cost_sigmoid_inflection_dist_mt}, {cost_sigmoid_inflection_slope_mt}')
			...
			pairings = rolling_hungarian_matching(ArUco_csv_path, slp_file_path, rolling_Hungarian_window_length, maximum_assignment_cost_threshold, cost_sigmoid_inflection_distance, cost_sigmoid_inflection_slope, min_track_detection_freq, cost_linear_dist_weight)
