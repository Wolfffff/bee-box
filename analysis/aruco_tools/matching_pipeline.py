...
def ArUco_SLEAP_matching(video_path: str, slp_predictions_path: str, start_end_frame: tuple, minimum_sleap_score: float = 0.1, crop_size: int = 50, half_rolling_window_size: int = 50, enhanced_output: bool = False, display_images_cv2: bool = False, sleap_predictions = None, sleap_instances = None, sleap_frames = None, tag_node = 0) -> np.ndarray:
	'''
	Args:
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] Starting assignment with frame range {start_end_frame}.')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP Data Restructuring] Started')
	sleap_interpolation_start = time.perf_counter()
	if (sleap_predictions is None) or (sleap_instances is None) or (sleap_frames is None):
		...
		logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] Received SLEAP file; moving immediately to processing')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP Data Restructuring] Ended, FPS: {round(float(start_end_frame[1] - start_end_frame[0] + 1) / float(sleap_interpolation_end - sleap_interpolation_start), 2)}')

	if enhanced_output:
		logger.info(sleap_predictions_df)

	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP-cropped ArUco] Started')
	ScA_start = time.perf_counter()

	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP-cropped ArUco] Initializing variables...')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP-cropped ArUco] Iterating through frames and running ArUco on crops')
	...
	if enhanced_output:
		logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] Frame {previous_frame}: {detections} tag(s)')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [SLEAP-cropped ArUco] Ended, FPS: {round(float(start_end_frame[1] - start_end_frame[0] + 1) / float(ScA_end - ScA_start), 2)}')

	if enhanced_output:
		logger.info(f'\nResults dataframe:\n{results_df}\n\n')

	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [Rolling Window Tag-Track Association] Started')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [Rolling Window Tag-Track Association] Filling initial window')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [Rolling Window Tag-Track Association] Starting Hungarian assignments with rolling window')
	...
	for center_of_window_frame in range(start_end_frame[0] + half_rolling_window_size + 1, start_end_frame[1] - half_rolling_window_size + 1):
		...
		for window_frame in range(center_of_window_frame - half_rolling_window_size, center_of_window_frame + half_rolling_window_size + 1):
			...
			for x in range(display_matrix.shape[0]):
				for y in range(display_matrix.shape[1]):
					if display_matrix[x, y] == '0':
						display_matrix[x, y] = ' '
			logger.info(tabulate(np.transpose(np.vstack([tags.astype(int), display_matrix])), tablefmt = 'pretty', headers = tracks))
			logger.info('\n')
			logger.info(f'Assigned tag-track pairs: {hungarian_pairs}')

	RWTTA_end = time.perf_counter()
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [Rolling Window Tag-Track Association] Ended, FPS: {round(float(start_end_frame[1] - start_end_frame[0] + 1) / float(RWTTA_end - RWTTA_start), 2)}')
	...
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] [Track Inheritance] Finished!')
	logger.info(f'[ArUco_SLEAP_matching {start_end_frame}] Done with SLEAP-based cropping ArUco tag-track association')
	...
	for frame in awpd.progressBar(frames_to_annotate, fill = 'X'):
		...
		for idx in range(current_frame_idx, next_frame_idx):
			...
			for pred_idx in range(prediction_start_idx, prediction_end_idx):
				...
				try:
					image = cv2.circle(image, (int(round(float(prediction_tuple[0]))), int(round(float(prediction_tuple[1])))), 10, pink, cv2.FILLED)
				except:
					errors += 1
			...
			try:
				...
				cv2.putText(image, str(int(current_track)), (pX, pY + 100), font, 2, green, 2)
			except:
		...
		for k in range(0,len(tags)):
			if last_seen[0, k] > 0 and last_seen[1, k] > 0:
				...
				cX = last_seen[0, k]
				cY = last_seen[1, k]
				cv2.putText(image, str(tags[k]), (last_seen[0, k], last_seen[1, k]), font, 2, red, 2)
...
def generate_final_output_dataframe(pairings: np.ndarray, output_path: str, start_end_frame = tuple, sleap_predictions = None, sleap_instances = None, sleap_frames = None, skeleton_dict: dict = {}) -> None:
	'''
	Args:
	...
	if (sleap_predictions is None) or (sleap_instances is None) or (sleap_frames is None):
		...
		logger.info(f'[Final Output Generation {start_end_frame}] Received SLEAP file; moving immediately to processing')
	...
	for frame in pairings[0, 1 : -1]:
		...
		for idx in range(current_frame_idx, next_frame_idx):
			...
			prediction = sleap_predictions[int(prediction_start_idx + skeleton_dict['tag'])]
			cX = float(prediction['x'])
			cY = float(prediction['y'])
			prediction = sleap_predictions[int(prediction_start_idx + skeleton_dict['head'])]
			headX = float(prediction['x'])
			headY = float(prediction['y'])
			prediction = sleap_predictions[int(prediction_start_idx + skeleton_dict['abdomen'])]
			abdomenX = float(prediction['x'])
			abdomenY = float(prediction['y'])
			prediction = sleap_predictions[int(prediction_start_idx + skeleton_dict['thorax'])]
				...
				if current_track in pairings[:, frame_idx]:
					...
					try:
						if int(current_tag) >= 0:
							output_data.append((frame, int(current_tag), cX, cY, headX, headY, thoraxX, thoraxY, abdomenX, abdomenY, Theta))
					except:
						pass
	
	output_df = pd.DataFrame(output_data, columns = ['Frame', 'Tag', 'cX', 'cY', 'headX', 'headY', 'thoraxX', 'thoraxY', 'abdomenX', 'abdomenY', 'Theta'])
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description = 'Import SLEAP data, locate ArUco tags, and output SLEAP tracks with corresponding ArUco tag.')
	parser.add_argument('video_path', help = 'The filepath of the video (preferably re-encoded as .mp4) to generate coordinates from as input, requires corresponding SLEAP (.slp) file.', type = str)
	parser.add_argument('slp_file_path', help = 'The filepath of the SLEAP (.slp) file to generate coordinates from, corresponding with the input video file.', type = str)
	parser.add_argument('files_folder_path', help = 'The filepath of a directory to save output files in. If the directory does not exist, it will be created, or failing that, an error will be thrown.', type = str)
	parser.add_argument('name_stem', help = 'A string to include as stems of filenames saved to files_folder_path.', type = str)
	parser.add_argument('start_here_frame', help = 'First frame in assignment.  NOTE: The first frame with data output is not this frame.  See comments in source code for more info.', type = int)
	parser.add_argument('end_here_frame', help = 'Last frame in assignment.  NOTE: The last frame with data output is not this frame.  See comments in source code for more info.', type = int)

	parser.add_argument('-a', '--annotate', help = 'Output a video with annotations (circles around bees with track and tag numbers; markings for ArUco tag detections).', action = 'store_true')
	parser.add_argument('-p', '--parallelize', help = 'Parallelize computation for more speed.  Only use for long runs (> 10^4 frames) because there is overhead associated with parallelization.  In shorter runs, single-threaded computation is faster and easier.', action = 'store_true')
	parser.add_argument('-d', '--display', help = 'Display cropped images of ArUco tags.  Useful for judging crop_size.  Only for local running, and never for actual large batches of data processing.', action = 'store_true')
	
	parser.add_argument('-ao', '--annotate_only', help = 'Skip matching step and annotate video based on previously saved pairings (*_matching_result.csv), the filepath to which must be passed along.', type = str, default = '')
	parser.add_argument('-v', '--verbosity', help = '0: Minimal, 1: Everything.  More options to be added (?)', type = int, choices = [0, 1], default = 0)
	parser.add_argument('-m', '--minimum_sleap_score', help = 'Minimum SLEAP prediction score for the data point to be used to look for an ArUco tag.  A crude way to try to avoid wasting time on bad SLEAP predicitons.', type = float, default = 0.1)
	parser.add_argument('-c', '--crop_size', help = 'The number of pixels horizontally and vertically around the SLEAP tag prediction point to crop away to run ArUco on.  Smaller values are faster, but risk cutting off tags and ruining perfectly good data.', type = int, default = 50)
	parser.add_argument('-w', '--half_rolling_window_size', help = "Used to specify the size of the rolling window for Hungarian matching. When SLEAP makes mistakes and passes a track from one bee to another, there's a transition region where the matching will not be guaranteed to follow perfectly; While the track transition is within the rolling window, we're not guaranteed to assign correctly. For instance, with half_rolling_window_size = 20 on a 20 fps video, we're not completely confident about the matching until 1 second before, and 1 second after the transition.", \
		type = int, default = 5)
	...
	logger.addHandler(logging.FileHandler(files_folder_path + '/' + name_stem + '_output_logs.log'))
	...
	if multithreaded and args.annotate_only == '':
		...
		for chunk in assignment_tuples:
			chunks_to_assign.append((video_path, slp_file_path, chunk, minimum_sleap_score, crop_size, half_rolling_window_size, False, False, sleap_predictions, sleap_instances, sleap_frames, skeleton_dict['tag']))
		...
		logger.info(f'[MAIN] Multiprocessed matching ended, effective overall FPS: {round(float(end_here_frame - start_here_frame + 1) / float(end - start), 2)}')
	...
	# If single-threaded
		...
		pairings = ArUco_SLEAP_matching(video_path, slp_file_path, (start_here_frame, end_here_frame), minimum_sleap_score, crop_size, half_rolling_window_size, enhanced_output, display_images_cv2, sleap_predictions, sleap_instances, sleap_frames, skeleton_dict['tag'])
		if enhanced_output:
	...
	if args.annotate_only == '':
		...
		try:
			...
			raise ValueError('The path to matching results for video annotation is incorrect')

	generate_final_output_dataframe(pairings, ArUco_csv_path, (start_here_frame, end_here_frame), sleap_predictions, sleap_instances, sleap_frames, skeleton_dict)
