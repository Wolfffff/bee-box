...
def create_confusion_matrix(
    aruco_csv_path: str, output_csv_path: str, checking_window: int
):
    ...
    frames_calculated = 0
    for frame in range(
        int(np.min(aruco_df.index)), int(np.max(aruco_df.index) - checking_window)
    ):
        frames_calculated += 1
        ...
        for row1, row2 in [(a, b) for a in window_iter for b in window_iter]:
            if row2.Tag != row1.Tag:
                distance_squared = float(
                    np.power(row1.cX - row2.cX, 2) + np.power(row1.cY - row2.cY, 2)
                )
                if distance_squared != 0:
                    confusion_matrix[index_dict[row1.Tag], index_dict[row2.Tag]] += (
                        1.0 / distance_squared
                    )
                    confusion_matrix[index_dict[row2.Tag], index_dict[row1.Tag]] += (
                        1.0 / distance_squared
                    )
...
create_confusion_matrix(
    "d:\\20210706_run000_00000000_aruco_annotated_30p_threshold.csv",
