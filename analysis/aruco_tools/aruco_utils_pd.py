...
def interpolate_empties(aruco_dict_by_tag: dict, tags: np.array) -> dict:
    ...
    for tag in tags:
        ...
        for current_frame in progressBar(
            frames_with_data, prefix="Tag #" + str(tag) + " interpolation", length=50
        ):
        ...
        aruco_dict_by_tag[tag] = pd.DataFrame(
            data=interp_data, columns=["cX", "cY"], index=indices
        )
...
def threshold_remove_bad_tags(
    aruco_df: pd.DataFrame, total_frames: int, percent_threshold: float
) -> pd.DataFrame:
    tags = find_tags_fast(aruco_df)
    aruco_dict_by_tag = sort_for_individual_tags(aruco_df, tags)
    _, _, _, _, _, captured_percents = collect_all_bee_stats(
        tags, aruco_dict_by_tag, total_frames, 1
    )
...
def collect_all_bee_stats(tags, aruco_df_by_tag, total_frames, fps):
    ...
    for tag_number in tags:
        ...
        captured_percents.append(
            float(len(aruco_df_by_tag[tag_number].index)) * 100.0 / total_frames
        )

    return tags, longest_skips, g1s_skips, g10s_skips, g100s_skips, captured_percents


def display_bee_stats_table(
    aruco_df: pd.DataFrame, aruco_dict_by_tag: dict, total_frames: int, fps: float
):
    """ """
    (
        tags,
        ...
        g100s_skips,
        captured_percents,
    ) = collect_all_bee_stats(aruco_df, aruco_dict_by_tag, total_frames, fps)
...
if __name__ == "__main__":
    aruco_df = load_into_pd_dataframe(
        "d:\\20210706_run000_00000000_aruco_annotated.csv"
    )
    ...
    aruco_df = threshold_remove_bad_tags(aruco_df, np.max(aruco_df.index), 30.0)
