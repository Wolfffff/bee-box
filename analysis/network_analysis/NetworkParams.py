...
for pos, edge_file in enumerate(os.listdir(sys.argv[1])):
        ...
        node_mapping = dict(zip(G2.nodes(), sorted(G2.nodes(), key=lambda k: random.random())))
        G2_relabeled = nx.relabel_nodes(G2, node_mapping)#This Randomizes the nodes for bootstrapping
        communities = sorted(nxcom.greedy_modularity_communities(G2_relabeled), key=len, reverse=True)
        #print(communities)
        combs = []
        if(pos==0):
