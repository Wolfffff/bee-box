...
def parser ():
        ...
        # Input arguments
        my_parser.add_argument('--dir', help = 'input directory with networks', type = str, action = confirmDir())
        my_parser.add_argument('--subgroup', help = 'subgroup of interest', type = str, action = confirmFile())
        my_parser.add_argument('--filter', help = 'nodes to not filter out', type = str, action = confirmFile())
        centtypes = ['betweenness', 'degree', 'eigen', 'closeness']
        my_parser.add_argument('--centrality', help = '', type = str, choices = centtypes, default = 'degree')
        delimtypes = [',','\t']
        my_parser.add_argument('--delim', help = '', type = str, choices = delimtypes, default = '\t')
        my_parser.add_argument('--thresh', help = '', type = float, default = 0.05)
        analtypes = ['all', 'centrality', 'assortativity', 'community']
        my_parser.add_argument('--analysis', help = 'which analyses you want', type = str, choices = analtypes, default = 'all')
...
def centrality(G2):
        ...
        plt.figure()
        nx.draw(G2,pos=None,with_labels=True,node_color=node_color,node_size=cent_size*2000,width=1)#,ax=fig.subplot(111))
...
for pos, edge_file in enumerate(os.listdir(myargs.dir)):
        ...
        if 'csv' not in edge_file: continue  
