...
coords_df.interpolate(method='linear', limit_area='inside', axis=0, limit=20, inplace=True, downcast='infer')
print(coords_df)

theta = np.arctan2(coords_df["headY"][args.frame_number] - coords_df["thoraxY"][args.frame_number], coords_df["headX"][args.frame_number] - coords_df["thoraxX"][args.frame_number])
print(f"\nCurrent frame theta: {theta}")

angular_resolution = 100
thetas_to_check = np.linspace(theta, theta + 2 * np.pi(), num=angular_resolution, endpoint=False)
...
for current_theta in thetas_to_check:
	...
	for shift in range(0, max_shift):
		cross_cor[shift, angular_index] = np.correlate(cX_array[args.frame_number:-1 - max_shift], cX_array)