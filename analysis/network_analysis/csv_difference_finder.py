...
print(
    f"CSV 1 missing frames: {np.setdiff1d(np.arange(np.min(frames1), np.max(frames1)), frames1)}"
)
print(
    f"CSV 1 missing frames: {np.setdiff1d(np.arange(np.min(frames2), np.max(frames2)), frames2)}\n"
)
...
for frame in np.intersect1d(frames1, frames2):
    ...
    for tag in frames_from_df1["Tag"].to_numpy():
        if str(frames_from_df1.loc[frames_from_df1["Tag"] == tag]) != str(
            frames_from_df2.loc[frames_from_df2["Tag"] == tag]
        ):
            differences += 1
