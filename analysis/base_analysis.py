...
bee_tracks_df = pd.read_csv(
    "d:\\bee_test\\files\\test_aruco_data_with_track_numbers.csv"
)
...
for bee_id in range(instance_count):
    plt.plot(
        bee_tracks[start_frame:end_frame, 0, bee_id],
        bee_tracks[start_frame:end_frame, 1, bee_id],
        alpha=0.1,
    )
# %%
import distinctipy
...
cap = cv2.VideoCapture("d:\\bee_test\\test.mp4")
...
] = "D:\\ffmpeg-4.4-essentials_build\\ffmpeg-4.4-essentials_build\\bin\\ffmpeg.exe"
ani = FuncAnimation(fig, animate, frames=range(100, 600)).save(
    "d:\\bee_test\\withframe.mp4",
    fps=20,
    bitrate=20000,
    dpi=300,
    writer="ffmpeg",
