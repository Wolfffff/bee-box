...
class SpatialRectangle:
    def __init__(self, *args, **kwargs):
        ...
        if len(args) != 2:
            raise Exception(f"Unable to assign SpatialRectangle from: {args}")

        # Confirm the coordinates are valid
        for coords in args:
            if len(coords) != 2:
                raise Exception(
                    f"Unable to assign SpatialRectangle coordinates from: {coords}"
                )
    ...
    def include(self, include_in, x_coords=None, y_coords=None):
        ...
        if isinstance(include_in, pd.DataFrame):
            ...
            return include_in[
                include_in[x_coords].between(self._x1, self._x2)
                & include_in[y_coords].between(self._y1, self._y2)
            ]
    ...
    def exclude(self, exclude_in, x_coords=None, y_coords=None):
        ...
        if isinstance(exclude_in, pd.DataFrame):
            ...
            return exclude_in[
                ~(
                    exclude_in[x_coords].between(self._x1, self._x2)
                    & exclude_in[y_coords].between(self._y1, self._y2)
                )
            ]
...
class SpatialTriangle:
    def __init__(self, *args, round_precision=1, **kwargs):
        ...
        # Confirm only two points were given
        if len(args) != 3:
            raise Exception(f"Unable to assign SpatialTriangle from: {args}")

        # Confirm the coordinates are valid
        for coords in args:
            if len(coords) != 2:
                raise Exception(
                    f"Unable to assign SpatialTriangle coordinates from: {coords}"
                )
        ...
        self.a = np.linalg.norm(
            np.array((self._x1, self._y1)) - np.array((self._x2, self._y2))
        )
        self.b = np.linalg.norm(
            np.array((self._x1, self._y1)) - np.array((self._x3, self._y3))
        )
        self.c = np.linalg.norm(
            np.array((self._x2, self._y2)) - np.array((self._x3, self._y3))
        )
        ...
        self.B = round(
            np.degrees(
                np.arccos(
                    (self.c ** 2 + self.a ** 2 - self.b ** 2) / (2 * self.c * self.a)
                )
            ),
            round_precision,
        )
    ...
    def plot(self):
        ...
        triangle_shape = Polygon(
            [(self._x1, self._y1), (self._x2, self._y2), (self._x3, self._y3)]
        )
...
class SpatialInteractor:
    def __init__(
        ...
        **kwargs,
        ...
        self._skeleton = LineString(
            [self._focal_coords, self._origin_coords, self._antifocal_coords]
        ).buffer(skeleton_buffer)

        # Create variables to store semicircles
        self._focal_semicircle = self._interactionSemicircle(
            self._origin_coords, self._focal_coords, semicircle_radius=semicircle_radius
        )
    ...
    def _interactor(self):
        ...
        if self._antifocal_semicircle != None:
            interactor_list.append(self._antifocal_semicircle)
    ...
    def fromTest(cls, origin_coords, focal_coords, antifocal_coords=[], **kwargs):
        ...
        if not antifocal_coords:
            antifocal_coords = origin_coords

        # Return the SpatialInteractor
        return cls(
            focal_coords=focal_coords,
            origin_coords=origin_coords,
            antifocal_coords=antifocal_coords,
            skeleton_buffer=0.01,
            semicircle_radius=1,
            **kwargs,
        )
    ...
    def fromBeeSeries(
        cls,
        bee_series,
        focal_cols=["headX", "headY"],
        origin_cols=["thoraxX", "thoraxY"],
        antifocal_cols=["abdomenX", "abdomenY"],
        centroid_cols=["cX", "cY"],
        **kwargs,
    ):
        ...
        try:
            bee_series.index = bee_series.index.droplevel(1)
        except:
            raise Exception(f"Unable to droplevel from series index: {bee_series}")

        # Return the SpatialInteractor
        return cls(
            focal_coords=bee_series[focal_cols].values,
            origin_coords=bee_series[origin_cols].values,
            antifocal_coords=bee_series[antifocal_cols].values,
            centroid_coords=bee_series[centroid_cols].values,
            **kwargs,
        )
    ...
    def fromBeeSeriesR(
        cls,
        bee_series,
        focal_cols=["thoraxX", "thoraxY"],
        origin_cols=["headX", "headY"],
        antifocal_cols=["abdomenX", "abdomenY"],
        centroid_cols=["cX", "cY"],
        **kwargs,
    ):
    ...
    def intersects(self, other_interactor):
        ...
        if self._interactor.intersects(other_interactor._interactor):
            line_style = "b-"
        else:
            line_style = "r-"
    ...
    def adjacent(self, other_interactor, cutoff=112.5):
		...
		"""
        adj_triangles.append(
            SpatialTriangle.fromCoords(
                self._focal_coords, self._origin_coords, other_interactor._focal_coords
            )
        )
        adj_triangles.append(
            SpatialTriangle.fromCoords(
                self._focal_coords,
                self._origin_coords,
                other_interactor._antifocal_coords,
            )
        )
        ...
        for adj_triangle in adj_triangles:
            ...
            """
            if max(adj_triangle.B, adj_triangle.C) <= cutoff:
                return True
    ...
    def plot(self, *args, **kwargs):
        ...
        if self._antifocal_semicircle:
            plt.plot(*self._antifocal_semicircle.exterior.xy, *args, **kwargs)

    def _nearParallel(
        self, other_interactor, cutoffs=((0, 45), (135, 225), (315, 360))
    ):
    ...
    def _interactionSemicircle(
        ...
        if not np.isnan(slope):
            ...
            extended_origin_x_coord = origin_coords[0] - line_extend_distance * (
                -1 if focal_less_than else 1
            )
            extended_origin_y_coord = extended_origin_x_coord * slope + intercept

            # Assign the theta of origin to focal
            theta = SpatialInteractor._arcTan2(
                origin_coords, focal_coords
            )  # np.arctan2(*(np.array(focal_coords) - np.array(origin_coords))[::-1])
        ...
        # Check if the line is vertical, i.e. undefined slope
            ...
            extended_origin_y_coord = origin_coords[1] - (
                line_extend_distance * (-1 if focal_less_than else 1)
            )

            # Assign the extended focal
            extended_focal_x_coord = focal_coords[0]
            extended_focal_y_coord = focal_coords[1] + (
                (semicircle_radius / 2) * (-1 if focal_less_than else 1)
            )

        # Assign the extended line
        extended_line_string = LineString(
            [(extended_origin_x_coord, extended_origin_y_coord), focal_coords]
        )
        ...
        left_border = rotate(
            extended_line_string, -angle_degrees, origin=Point(focal_coords)
        )
        right_border = rotate(
            extended_line_string, angle_degrees, origin=Point(focal_coords)
        )
        ...
        if split_interaction_circle.geoms[1].contains(extended_focal_coords):
            interaction_semicircle = split_interaction_circle.geoms[1]
        else:
            interaction_semicircle = split_interaction_circle.geoms[0]
